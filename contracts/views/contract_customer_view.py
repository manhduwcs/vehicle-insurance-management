...
def contract_list(request):
    ...
    if not group_id or not has_permission(
        group_id, FunctionIds.ManageContractsByCustomers, ActionIds.View
    ):
    ...
    contracts = list(
        Contracts.objects.select_related(
            "vehicle", "insurance_category", "duration", "created_by"
        )
...
def contract_detail(request, pk):
    ...
    can_edit = group_id and has_permission(
        group_id, FunctionIds.ManageContractsByCustomers, ActionIds.Edit
    )
...
def goto_payment_choice(request, pk):
    ...
    context = {"segment": "contracts", "contract": contract}
...
def payment_detail(request, pk):
    contract = get_object_or_404(
        Contracts.objects.select_related("vehicle__customer_id"), pk=pk
    )
...
def pay_with_card(request, pk):
    ...
    if request.method == "POST":
        ...
        contract.save(
            update_fields=["payment_type", "payment_amount", "payment_at", "status"]
        )
...
def list_insurance_categories(request):
    if request.method == "POST":
        ...
        if not vehicles.exists():
            messages.warning(
                request,
                "You need to register your vehicle on the system before purchasing insurance.",
            )
            return render(
                request,
        ...
        if category_id == "1":
            return redirect(
                "contracts_customer:create_contract_civil", category_id=category_id
            )
        return redirect(
            "contracts_customer:create_contract_other", category_id=category_id
        )

    return render(
        request,
        "contracts/customer/list_category.html",
        {"categories": InsuranceCategories.objects.all()},
    )
...
def create_contract_civil(request, category_id):
    ...
    group_id = request.session.get("group_id")
    if not group_id or not has_permission(
        group_id, FunctionIds.ManageContractsByCustomers, ActionIds.Create
    ):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            price_list = InsurancePriceList.objects.filter(
                insurance_category_id=insurance_category_id, duration=duration_id
            ).first()
            if not price_list:
                messages.error(
                    request, "No insurance available for this vehicle age and duration."
                )
                return render(
                    request,
                    "contracts/customer/create_civil.html",
                    {
                        "form": form,
                        "durations": durations,
            ...
            messages.success(
                request, "You have successfully registered to buy insurance!"
            )
            return redirect("contracts_customer:contract_list")
        else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not price_list:
                ...
                return render(
                    request,
                    "contracts/customer/create_other.html",
                    {
                        "form": form,
                        "durations": durations,
                        "category": InsuranceCategories.objects.get(id=category_id),
            ...
            rate_percentage = Decimal(str(price_list.rate)) / Decimal("100")
            max_coverage_percentage = Decimal(
                str(price_list.max_coverage_rate)
            ) / Decimal("100")
            duration_factor = Decimal(str(duration_id.months)) / Decimal("12")
            estimate_premium = (
                vehicle.purchase_price
                * Decimal(str(depreciation.Rate))
                * rate_percentage
                * duration_factor
            )
            estimate_value = (
...
def calculate_insurance(request):
    ...
    try:
        vehicle = Vehicles.objects.select_related("vehicle_type").get(
            id=vehicle_id, customer_id=request.session["user_id"]
        )
        category = InsuranceCategories.objects.get(id=category_id)
        durations = Duration.objects.all()
        current_date = datetime.now().date()
        ...
        for duration in durations:
            item = {
                ...
                "months": float(duration.months),
                "available": True,
            }

            if int(category_id) == 1:  # Civil liability
                price_list = InsurancePriceList.objects.filter(
                    insurance_category_id=category_id, duration=duration
                ).first()
                if price_list:
                    item["estimate_premium"] = float(
                        (vehicle.vehicle_type.fee * price_list.rate)
                        / 100
                        * duration.months
                        / 12
                    )
                    item["max_person_compensation"] = float(
                        vehicle.vehicle_type.max_personal_compensation
                    )
                    item["max_property_compensation"] = float(
                        vehicle.vehicle_type.max_property_compensation
                    )
                else:
                ...
                print(
                    f"Processing category {category_id}, age {age}, duration {duration.id}"
                )  # Debug
                if age > 20:
                    item["available"] = False
                    ...
                    if not depreciation:
                        ...
                        if price_list:
                            ...
                            rate_percentage = Decimal(str(price_list.rate)) / Decimal(
                                "100"
                            )
                            max_coverage_percentage = Decimal(
                                str(price_list.max_coverage_rate)
                            ) / Decimal("100")
                            duration_factor = Decimal(str(duration.months)) / Decimal(
                                "12"
                            )
                            item["estimate_premium"] = float(
                                vehicle.purchase_price
                            ...
                            item["available"] = False
                            item["error"] = (
                                "No matching price list for this duration and age."
                            )
